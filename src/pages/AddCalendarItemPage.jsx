...
function AddCalendarItemInner() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (res.ok) {
      addToast('Calendar item created', 'success');
      setTimeout(() => navigate('/admin/calendar'), 800);
    } else addToast('Failed to create item', 'error');
    setSaving(false);
  };
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, ease: [0.22, 1, 0.36, 1] }}>
        ...
        <form onSubmit={handleSubmit} className="glass rounded-2xl border border-white/[0.06] p-5 md:p-6 space-y-5">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <FormField label="Type"><Select value={form.type} onChange={e => setForm(f => ({ ...f, type: e.target.value }))} options={[{ value: 'event', label: 'Event' }, { value: 'note', label: 'Note' }, { value: 'reminder', label: 'Reminder' }, { value: 'holiday', label: 'Holiday' }]} /></FormField>
            ...
            <FormField label="Description" className="md:col-span-2"><Textarea value={form.description} onChange={e => setForm(f => ({ ...f, description: e.target.value }))} rows={2} /></FormField>
            <FormField label="Location"><Input type="text" value={form.location} onChange={e => setForm(f => ({ ...f, location: e.target.value }))} /></FormField>
          ...
          <FormActions saving={saving} saveLabel="Create Item" onCancel={() => navigate('/admin/calendar')} />
        </form>
      </motion.div>
    </div>
  );
